...
void Dao::insertPhysician(QString name, QString &status)
{
    QString qry = "INSERT INTO PHYSICIAN (name)"
                  "VALUES (:name)";
    ...
    if(! query.exec() ){
        ...
        status = "Physician Inserted Successfully";
    }
}

void Dao::insertRefPhysician(QString name, QString &status)
{
    QString qry = "INSERT INTO REFPHYSICIAN (name)"
                  "VALUES (:name)";
    ...
    if(! query.exec() ){
        //status = query.lastError().text();
        status = "This Ref. Physican Name is Already Existing";
    }
    else{
        status = "Ref. Physician Inserted Successfully";
    }
}

void Dao::insertDepartment(QString name, QString &status)
{
    QString qry = "INSERT INTO DEPARTMENT (name)"
                  "VALUES (:name)";
    ...
    if(! query.exec() ){
        //status = query.lastError().text();
        status = "This Department Name is Already Existing";
    }
    else{
...
void Dao::insertExamtype(QString type, QString &status)
{
    QString qry = "INSERT INTO EXAMTYPE (type)"
                  "VALUES (:type)";
    ...
    if(! query.exec() ){
        //status = query.lastError().text();
        status = "This Exam Type is Already Existing";
    }
    else{
